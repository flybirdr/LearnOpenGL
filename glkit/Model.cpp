...
glkit::Model::Model(const std::string &modelFileName)
{
    mDirectory = modelFileName.substr(0, modelFileName.find_last_of('/') + 1);
    loadModel(modelFileName);
}

void glkit::Model::loadModel(const std::string &modelFileName)
{
...
void glkit::Model::processNode(const aiScene *sence, aiNode *node)
{
...
Mesh glkit::Model::processMesh(const aiScene *sence, aiMesh *mesh)
{
...
std::vector<Texture> glkit::Model::loadTextures(const aiMaterial *material)
{
    std::vector<Texture> textures;
    aiString name;
    bool skip = false;
...
void glkit::Model::draw(Shader &shader)
{
...
std::vector<Mesh>& glkit::Model::getMeshes()
{
    return mMeshes;
}
